...
const (
	...
	OPTIONS_AUTH_TAG="nginx.ingress.kubernetes.io/auth-enable"
	OPTIONS_RATELIMIT_TAG="nginx.ingress.kubernetes.io/limit-enable"
)
//@service
...
func(this *IngressService) PostIngress(post *models.IngressPost,isupdate bool) error{
	className := "nginx"
	...
	if isupdate{
		_,err:= this.Client.NetworkingV1beta1().Ingresses(post.Namespace).
			Update(context.Background(),ingress,v1.UpdateOptions{})
		return err
	}else{
		_,err:= this.Client.NetworkingV1beta1().Ingresses(post.Namespace).
...
func(this *IngressService) getIngressOptions(t int,item *v1beta1.Ingress) bool{
	...
	case OPTION_AUTH:
		//fmt.Println(item.Annotations)
		if _,ok:=item.Annotations[OPTIONS_AUTH_TAG];ok{
			return true
		}
	case OPTION_RATELIMIT:
	//	fmt.Println(item.Annotations)
		if _,ok:=item.Annotations[OPTIONS_RATELIMIT_TAG];ok{
			return true
		}
	}
	...
	for i,item:=range list{
		ret[i]=&models.IngressModel{
			...
			Options:models.IngressOptions{
				...
				IsAuth:this.getIngressOptions(OPTION_AUTH,item),
				IsRatelimit:this.getIngressOptions(OPTION_RATELIMIT,item),
			},
		}
	}
	return ret
